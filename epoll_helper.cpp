...
void Server::launch( const int&         port,
  ...
  // In the loop, monitor events and if event on server socket, add new client
  // to epoll, else, handle client event
  while( g_running ) {
    ...
    for( int i = 0; i < numEvents; i++ ) {
      ...
      if( events[i].data.fd == _serverSocket ) {
        ...
        if( buffer.find( '\n' ) != std::string::npos ) {
          ...
          // if the client is authentificated (PASS NICK USER) and not
          // RPL_WELCOMEd
          try {
...
// Create server socket and set options to ensure port is reusable if server is
// restarted.
...
void Server::listenServerSocket() {
  if( listen( _serverSocket, MAXCONNECTED ) == -1 )
    throw std::runtime_error(
      "Could not set server socket to listening state" );
}
...
void Server::acceptNewConnection( int& clientSocket ) {
  ...
  clientSocket = accept( _serverSocket, (struct sockaddr*)&clientAddress,
                         &clientaddrlen );
}
...
void Server::addClient( int fd, Client client ) {
  ...
  std::cout << YELLOW_BLOC << " + " << RESET << DIM << " Client connecting "
            << RESET << std::endl
            << std::endl;
  std::cout << BOLD << "Socket:\t\t" << RESET << client.getClientSocket()
            << std::endl;
  _clients[fd] = client;
...
bool Server::handleNewClient( int& clientSocket ) {
  ...
  if( clientSocket == -1 && g_running ) {
    std::cout << BRED << "Error" << RESET << ": Could not connect client."
              << std::endl;
    return ( false );
  }
  if( _clients.size() >= MAXCONNECTED ) {
    std::cout << BRED << "Error" << RESET
              << ": Too many clients connected already" << std::endl;
    sendNumericReplies(
      1, clientSocket,
      std::string( ":" + _serverName
                   + " 999  :Too many clients connected already\r\n" )
        .c_str() );
...
bool Server::requestIsComplete( const int&   clientSocket,
  ...
  if( received <= 0 ) {
    ...
    clientToRemove->quitServer( "disconnected",
                                &( clientToRemove->getChannelsMap() ) );
...
void Server::displayRequest( int& requestIndex, const std::string& buffer ) {
  std::cout << HIGHLIGHT << BOLD << " #" << ++requestIndex << " " << RESET
            << DIM << " Request received " << RESET << std::endl
            << std::endl;
...
void Server::welcomeClient( const int& clientSocket ) {
  ...
  sendNumericReplies(
    6, clientSocket, RPL_WELCOME( _serverName, nickname ).c_str(),
  ...
  while( !requestStream.eof() ) {
    ...
    if( _commands.find( command ) != _commands.end() ) {
      ...
      if( command != "PASS" && client.getPassword().empty() )
        continue;
      if( client.isAuthentificated() || command == "NICK" || command == "USER"
          || command == "PASS" )
